...
export const TELEPORT_SHELL_SNIPPET = [
  ...
  '  command tasks start "$@"',
  '  local status=$?',
  '  # Grab the last positional (the task id) portably: this loop leaves',
  '  # task_id holding the final argument in both bash and zsh, avoiding the',
  '  # bash-only `${@: -1}` slice that silently yields empty under `emulate sh`.',
  '  local task_id=',
  '  for task_id; do :; done',
  '  local destination',
