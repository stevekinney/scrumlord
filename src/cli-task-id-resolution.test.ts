...
describe('resolveTaskId — `current` token', () => {
  ...
  it('add-tag current adds a tag to the current branch task', async () => {
    ...
    const result = await runTasksCli(['add-tag', 'current', 'tagged-via-current'], { cwd: root });
    expect(JSON.parse(result.stdout)).toMatchObject({
      id: currentId,
  ...
  it('remove-tag current removes a tag from the current branch task', async () => {
    const root = await workspaceRoot();
    const { currentId } = await seedTasks(root);

    await runTasksCli(['add-tag', 'current', 'tag-to-remove'], { cwd: root });
    const result = await runTasksCli(['remove-tag', 'current', 'tag-to-remove'], { cwd: root });
  ...
  it('add-blocker current <uuid> adds the specified task as blocker of the current task', async () => {
    ...
    const result = await runTasksCli(['add-blocker', 'current', blockerId], { cwd: root });
    expect(JSON.parse(result.stdout)).toMatchObject({
      id: currentId,
      blockedBy: [blockerId],
    });
  ...
  it('blocking current lists tasks the current branch task blocks', async () => {
    ...
    await runTasksCli(['add-blocker', dependentId, currentId], { cwd: root });

    const result = await runTasksCli(['blocking', 'current'], { cwd: root });
    const dependents = JSON.parse(result.stdout) as { id: string }[];
    expect(dependents.some((t) => t.id === dependentId)).toBe(true);
  });
