...
- Store the Git branch on tasks with `tasks update current --branch <branch>` when work is branch-bound. Setting a branch moves a `draft` or `ready` task to `in-progress`. The branch must be a feature branch — Scrumlord rejects integration branches (`main`, `master`) with `invalid_branch`. For agent-owned work, let `tasks start`/`tasks next` create the `tasks/<short-id>` branch rather than setting one by hand.
