...
3. **Inventory Git state.** \`git branch --list 'tasks/*'\`, \`git worktree list --porcelain\`, \`git for-each-ref --format='%(refname:short) %(committerdate:iso)' refs/heads refs/remotes/origin\`.
...
6. **Detect orphan tasks (use the per-task atomic CLI).** Do **not** combine \`--orphans-only\` with \`--recover-orphans\`, \`--hard\`, or \`<days>\` — those combinations error. Preview with \`tasks cleanup --orphans-only --dry-run\` and read the report. If the candidates look correct, run \`tasks cleanup --orphans-only\` to apply them. The command always exits 0; parse its stdout to see recovered vs. skipped counts and record both in the report. Do **not** chain \`tasks set-status\`/\`tasks clear-branch\`/\`tasks clear-session\` by hand — those are three separate writes per task and would leave half-applied state on failure.
7. **Detect orphan branches (report-only).** Branches matching \`tasks/*\` with no task pointing at them and no open PR. List them in **Suggested follow-ups**. Do not delete.
...
11. **Audit blockers.** \`tasks blocked\`. For each edge: confirm the blocker exists and isn't soft-deleted. Stale ⇒ \`tasks blockers remove\`. Cycles ⇒ flag in **Open questions**, do not auto-break.
