...
Parse the JSON. Confirm the task is `in-progress` and has a `branch`. This is the task `tasks prompt next` claimed — adopt its `id` as `$TASK_ID` for the rest of the run. If `tasks current` returns nothing or errors with `current_task_ambiguous`, inspect `tasks with-branch "$(git branch --show-current)"` and choose explicitly before continuing.
